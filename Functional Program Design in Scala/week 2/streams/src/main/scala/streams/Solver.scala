...
import scala.collection.immutable.LazyList.concat
...
trait Solver extends GameDef:
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    initial match
      ...
      case (block, moves) #:: rest =>
        val frontier = newNeighborsOnly(neighborsWithHistory(block, moves), explored)
        (block, moves) #:: from(rest ++ frontier, explored + block)

  /**
   * The lazy list of all paths that begin at the starting block.
   */
  lazy val pathsFromStart: LazyList[(Block, List[Move])] =
    from(LazyList((startBlock, List.empty)), Set())
  ...
  lazy val solution: List[Move] =
    pathsToGoal match
      case LazyList() => List.empty
      case path #:: _ => path._2.reverse
